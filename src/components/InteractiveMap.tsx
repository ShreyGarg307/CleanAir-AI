...
import { Search, AlertTriangle, Factory, Building2, Server, Crosshair, Wind } from 'lucide-react';
...
export function InteractiveMap() {
  ...
  const { aqiIndex, aqiCategory, dominantPollutant } = useAqi();
  ...
  useEffect(() => {
    if (!isLoaded || !mapInstance || !window.google) return;
    
    if (showWindLayer) {
      const windTileLayer = new window.google.maps.ImageMapType({
        getTileUrl: function (coord, zoom) {
          return `https://tile.openweathermap.org/map/wind_new/${zoom}/${coord.x}/${coord.y}.png?appid=${WEATHER_KEY}`;
        },
      ...
      mapInstance.overlayMapTypes.push(windTileLayer);
    } else {
      mapInstance.overlayMapTypes.clear();
    }
  }, [showWindLayer, mapInstance, isLoaded]);
  ...
  // AQI zone markers — derived from real aqiIndex with deterministic zone offsets
  const ZONE_OFFSETS = [
    ...
    { id: 'Western Zone',  angleFrac: 1.00, radius: 0.040, offset: -12 },
  ];

  const aqiMarkers = useMemo(() => {
    if (!aqiIndex) return [];
    return ZONE_OFFSETS.map(zone => ({
      id: zone.id,
      position: {
    ...
    <div className="flex flex-col w-full h-full rounded-2xl overflow-hidden shadow-sm border border-slate-200 bg-white relative">
      ...
      <div className="relative flex-1 w-full min-h-0 bg-slate-100">
        ...
        >
          ...
          {/* City AQI Overlay — powered by AqiContext (Google Air Quality API) */}
          {aqiIndex > 0 && (
            <InfoWindow
              position={coordinates}
              options={{ disableAutoPan: true }}
            >
              <div className="bg-white rounded-xl shadow-lg p-4 min-w-[170px]">
                <div className="flex flex-col items-center justify-center mb-2">
                  <span className={`text-5xl font-black ${
                    aqiIndex <= 100 ? 'text-green-500' :
                    aqiIndex <= 200 ? 'text-orange-500' :
                    'text-red-500'
                  }`}>
                    {aqiIndex}
                  </span>
                  <span className={`text-sm font-bold uppercase tracking-wider mt-1 ${
                    aqiIndex <= 100 ? 'text-green-600' :
                    aqiIndex <= 200 ? 'text-orange-600' :
                    'text-red-600'
                  }`}>
                    {aqiCategory}
                  </span>
                ...
                <div className="text-center pt-2 border-t border-slate-100">
                  ...
                  <p className="text-xs font-bold text-slate-800 mt-0.5">{dominantPollutant}</p>
                </div>
              </div>
            </InfoWindow>
          )}
          ...
          {layers.incidents && incidentReports.map((report) => {
            let iconColor = 'text-yellow-500';
            const upvotes = report.initialUpvotes || 0;
            if (upvotes >= 10 && upvotes < 50) iconColor = 'text-orange-500';
            if (upvotes >= 50) iconColor = 'text-red-500';
            
            // Ensure lat and lng are floats
            const pos = { 
              lat: parseFloat(report.location?.lat || report.lat), 
              lng: parseFloat(report.location?.lng || report.lng) 
            };
          ...
          {activeReport && (
             <InfoWindow
               position={{ 
                 lat: parseFloat(activeReport.location?.lat || activeReport.lat), 
                 lng: parseFloat(activeReport.location?.lng || activeReport.lng) 
               }}
               onCloseClick={() => setActiveReport(null)}
             >
               <div className="p-2 max-w-[240px] text-slate-800">
                  {activeReport.imageUrl && (
                    <img src={activeReport.imageUrl} alt="Incident" className="w-full h-24 object-cover rounded-t-md mb-2" />
                  )}
                  <h3 className="text-sm font-bold mb-1">{activeReport.category}</h3>
                  <p className="text-xs font-medium mb-2 line-clamp-3 leading-tight">{activeReport.description}</p>
                  
                  <div className="flex items-center justify-between mt-2">
                    <span className="text-[10px] font-bold uppercase px-2 py-0.5 rounded-full bg-slate-100 border border-slate-200">
                      {activeReport.status || 'Pending'}
                    </span>
                    <span className="text-[10px] font-bold text-rose-600 bg-rose-50 px-2 py-0.5 rounded-full">
                      {activeReport.initialUpvotes || 0} upvotes
                    </span>
                  </div>
               </div>
             </InfoWindow>
      ...
      <div className="flex flex-col items-center justify-center flex-1 w-full bg-slate-100 p-8">
        <div className="animate-pulse flex flex-col items-center">
          <div className="w-12 h-12 border-4 border-emerald-500 border-t-transparent rounded-full animate-spin mb-4"></div>
